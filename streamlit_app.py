...
<div style="font-family: sans-serif; text-align: justify; font-size: 14px; line-height: 1.6;">
    ...
    <div style="display: inline-flex; align-items: center;">
        <svg height="15" width="15">
            <circle cx="7" cy="7" r="3" fill="orange" />
        </svg>
        Rescue Centers
    </div>
</div>
...
This map is our starting point, displaying the concentration of accidents and rescue centers across Paris. Yellow dots represent the rescue centers, anchoring the web of assistance that spans the city. The purple circles indicate the main zones of incidents, where the frequency and gravity of accidents necessitate focused attention. Linking these points, we see the paths of intervention, illustrating the dynamic flow of aid dispatched throughout the day. By filtering the data by cause or time, we can observe the distribution and frequency of interventions during specific hours. This visualization is crucial for understanding how resources are allocated in times of need and where the hotspots for accidents are located within the city.
</div>
"""
...
justified_text2 = """
<div style='text-align: justify;'>
 Next, we examine the brigade's efficiency through the lens of response times. The bar graph here quantifies how quickly different vehicle types are able to get on the move after an alert. Adjacent to it, the pie chart ranks the most frequently deployed vehicles, giving us insight into which units are most commonly in the field. Together, these charts provide a comprehensive look at the fleet's preparedness and the types of vehicles that form the backbone of emergency response in Paris.</div>
"""
...
global_view_chart = alt.Chart(filtered_data).mark_line(point=True).encode(
    x=alt.X('hour of day:O', title='Hours of a day'),
...
tooltip=[
        alt.Tooltip('hour of day', title='Hour of the day'),
